...
export default {
    ...
    computed: {
        ...
        stackedData() {
            ...
            return this.data.map((yearData , index) => {
                ...
                const offset = this.base - (total / 2);
                let accumulate = 0;
                this.names.forEach(name => {
                    const value = yearData[name];
                    stacked[`${name}_bottom`] = offset + accumulate;
                    accumulate += value;
                    stacked[`${name}_top`] = offset + accumulate;
                });
                return stacked;
            });
        }
    ...
    methods: {
        ...
        getYposition(value) {
            ...
            const ratio = value / this.maxYvalue;
